...
<%@ page import="com.wei.entity.*" %>
<%@ page import="com.wei.dao.CartDao" %>
...
<head>
    ...
    <style>
        .fengexian{ border-top:1px solid #ddd; height:12px; background: #ECFBD7; text-align:center; line-height:12px; padding:0px 10px; color: #333}
    </style>
    <title>用户购物车</title>
</head>
...
<%
    ...
    List<Cart> cart_list = (List<Cart>) httpSession.getAttribute("cart_list");
    if (cart_list.size() == 0) {
        String msg = "当前购物车为空！";
...
<div class="user_style clearfix">
    <div class="user_center clearfix">
        ...
        <div class="right_style">
            <div class="title_style"><em></em>浏览记录</div>
            <div class="Order_form_style">
                <div class="Order_form_list">
                    <form action="UsersServlet?action=purchase_product" method="post">
                        <table>
                            <thead>
                            <tr>
                                <td class="list_name_title0">商品名称</td>
                                <td class="list_name_title1">商品图像</td>
                                <td class="list_name_title2">商品描述</td>
                                <td class="list_name_title3">商品单价（￥）</td>
                                <td class="list_name_title4">商品数量</td>
                            </tr>
                            </thead>
                        <%
                            for (Cart cart : cart_list){
                                Product product = productDao.select_by_pro_id(cart.getCart_pro_id());
                                String pro_image = product.getPro_image();
                                String pro_id = Integer.toString(product.getPro_id());
                        %>
                            <tbody>
                            <tr><td colspan="6" class="fengexian" align="center">~~~~~我是可爱的分割线~~~~~<em></em></td></tr>
                            <tr class="Order_Details">
                                ...
                                <td class="split_line" width="37.5%"><%=product.getPro_info()%></td>
                                <td width="12.5%"><%=product.getPro_price()%></td>
                                <td width="12.5%"><input style="width:70px" type="number" name="purchase_pro_id_<%=product.getPro_id()%>" value="<%=cart.getCart_pro_number()%>"/></td>
                            </tr>
                            </tbody>
                            <%
                                }
